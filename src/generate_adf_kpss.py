...
def run_adf(x, lag=None, regression="c"):
    ...
    _icbest = result[5] if len(result) == 6 else float("nan")
...
def main():
    ...
    ap.add_argument(
        "--col", required=True, help="Nombre de la columna numÃ©rica a analizar."
    )
    ap.add_argument("--out", required=True, help="Ruta de salida del CSV completo.")
    ap.add_argument(
        "--maxlag", type=int, default=5, help="MÃ¡ximo lag explÃ­cito a evaluar."
    )
    ap.add_argument(
        "--trend",
        default="c",
        choices=["c", "ct"],
        help="RegresiÃ³n: c=const, ct=const+trend.",
    )
    args = ap.parse_args()
    ...
    rows.append(
        {
            "series": args.col,
            "transform": "level",
            **run_adf(x, lag=None, regression=args.trend),
        }
    )
        ...
        rows.append(
            {
                "series": args.col,
                "transform": "level",
                **run_adf(x, lag=lag, regression=args.trend),
            }
        )
    ...
    rows.append(
        {
            "series": args.col,
            "transform": "level",
            **run_kpss(x, lag=None, regression=args.trend),
        }
    )
    for lag in range(0, args.maxlag + 1):
        rows.append(
            {
                "series": args.col,
                "transform": "level",
                **run_kpss(x, lag=lag, regression=args.trend),
            }
        )
    ...
    rows.append(
        {
            "series": args.col,
            "transform": "diff1",
            **run_adf(dx, lag=None, regression=args.trend),
        }
    )
    for lag in range(0, args.maxlag + 1):
        rows.append(
            {
                "series": args.col,
                "transform": "diff1",
                **run_adf(dx, lag=lag, regression=args.trend),
            }
        )
    ...
    rows.append(
        {
            "series": args.col,
            "transform": "diff1",
            **run_kpss(dx, lag=None, regression=args.trend),
        }
    )
    for lag in range(0, args.maxlag + 1):
        rows.append(
            {
                "series": args.col,
                "transform": "diff1",
                **run_kpss(dx, lag=lag, regression=args.trend),
            }
        )
    ...
    # Resumen corto (lags 0,1,2) Ãºtil para Tabla A-1

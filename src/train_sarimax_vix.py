...
for fold in range(1, end + 1):
    ...
    preds = model.predict(start=y_val.index[0], end=y_val.index[-1], exog=X_val)
    ...
    pred_records.append(
        pd.DataFrame(
            {"Date": y_val.index, "y_true": y_val.values, "y_pred": preds.values}
        )
    )
...
json.dump(
    {"order": ORDERS, "seasonal_order": SEAS_ORDERS, "exog": EXOG_COLS},

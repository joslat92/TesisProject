...
with open(LOG_PATH, "w", newline="") as f:
    csv.writer(f).writerow(list(param_grid.keys()) + ["val_RMSE", "test_RMSE"])
...
for combo in grid_iter:
    ...
    early = EarlyStopping(
        monitor="val_loss", patience=EARLY_STOP, restore_best_weights=True, verbose=0
    )
...
def _product_values(pg: Dict[str, Sequence[Any]]) -> Iterable[tuple[Any, ...]]:
    """Mypy-safe: convierte dict_values en listas y expande con *args."""
    vals: list[Sequence[Any]] = [list(v) for v in pg.values()]
    return product(*vals)

...
Usage (from project root):
    python src/train_sarima.py --train_len 1500 --test_len 20 --stride 20 \
                               --target diff --out_dir models/SARIMA
"""
...
parser.add_argument("--out_dir", default="models/SARIMA")
...
price: pd.Series = df["Target_Price"].astype(float)

if args.target == "diff":
    y_series: pd.Series = price.diff().dropna()
else:  # log-ret
    y_series = np.log(price).diff().dropna()

# Align features to target index (no exógenas en este baseline)
y_df: pd.DataFrame = y_series.to_frame("y")

# ---------- Rolling training ----------
fold_preds = []
for fold, (train_df, test_df) in enumerate(
    rolling_splits(y_df, args.train_len, args.test_len, args.stride), 1
):
...
print("✅ SARIMA rolling-backtest completado y resultados guardados")

...
# --- ParÃ¡metros globales del experimento ---
N_STEPS = 40
TRAIN_END = "2023-12-31"
TEST_START = "2024-01-01"
DATA_PATH = (
    "data/df_final_ready_plus_vix.csv"  # AsegÃºrate de que esta ruta sea correcta
)
...
# --- FunciÃ³n para precisiÃ³n direccional ---
...
train_vals = train_df["Target_Price"].to_numpy().reshape(-1, 1)
test_vals = test_df["Target_Price"].to_numpy().reshape(-1, 1)
...
# Se necesita un "seed" con los Ãºltimos N_STEPS del entrenamiento para predecir el primer punto del test
...
print(f"Forma de X_test para predicciÃ³n: {X_test.shape}")
...
# 7. Guardar resultados y grÃ¡fico
out_dir = Path("models/LSTM_HYBRID_WF_ROBUST")
out_dir.mkdir(parents=True, exist_ok=True)

results_df = pd.DataFrame(
    {"Date": test_df.index[N_STEPS:], "y_true": y_test_inv, "y_pred": y_pred_inv}
)
results_path = out_dir / "pred_walkforward.csv"
results_df.to_csv(results_path, index=False)
print(f"\nâœ… Predicciones guardadas en: {results_path}")
...
plt.plot(
    results_df["Date"],
    results_df["y_pred"],
    label="Predicho (LSTM Hybrid)",
    alpha=0.8,
    linestyle="--",
)
plt.title("Prueba de Robustez Walk-Forward (2024-2025) â€“ LSTM Hybrid")
...
print(f"âœ… GrÃ¡fico guardado en: {fig_path}")
plt.show()

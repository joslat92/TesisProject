...
def load_all_predictions(models_dir: Path) -> pd.DataFrame:
    ...
    truth_df = pd.read_csv(
        MASTER_DATA_PATH, usecols=["Date", "Target_Price"], parse_dates=["Date"]
    )
    ...
    for csv_path in models_dir.glob("*/predictions_all_folds.csv"):
        ...
        try:
            ...
            if not y_pred_col:
                print(
                    f"⚠️  Ignorado '{model_name}': no se encontró la columna de predicción ('y_pred' o 'Pred')."
                )
                continue
...
def plot_time_series_and_boxplot(df: pd.DataFrame, window: int, out_dir: Path):
    ...
    price_models = [
        ...
        if grp["y_pred"].mean() > main_scale_mean * 0.5
    ]
    ...
    plt.ylabel("Precio NASDAQ-100", fontsize=12)
    plt.legend(loc="upper left")
    plt.tight_layout()
    plt.savefig(out_dir / "time_series_final.png", dpi=300)
    plt.close()
    print(
        f"✅ Gráfico de series de tiempo final guardado en '{out_dir / 'time_series_final.png'}'"
    )
    ...
    sorted_data = [
        df_price[df_price["model"] == model]["abs_err"].dropna().values
    ...
    plt.close()
    print(
        f"✅ Gráfico de cajas de error final guardado en '{out_dir / 'error_boxplot_final.png'}'"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Genera gráficos de resultados para la tesis."
    )
